...
def delete_user(user_id):
    ...
    # Samo admin ili sam korisnik može obrisati svoj račun
    if current_user.id != user_id and getattr(current_user, "role", "user") != "admin":
        flash("Nemate ovlasti za brisanje ovog korisnika.", "danger")
        return redirect(url_for("auth.profile"))
...
def admin_dashboard():
    ...
    for u in raw_users:
        u["id"] = str(u["_id"])   # dodaj string verziju
        users.append(u)
...
def set_role(user_id, role):
    ...
    if getattr(current_user, "role", "user") != "admin":
        flash("Nemate ovlasti za promjenu role.", "danger")
